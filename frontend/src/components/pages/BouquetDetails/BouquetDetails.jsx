...
const BouquetDetails = ({ bouquets, bouquetComponents, componentPrices, reviews, users, cartItems, setCartItems }) => {
    const { id } = useParams();
    ...
    if (!bouquet || bouquet.deleted_at) {
        return <div className="bouquet-details-error">Букет не найден. Я запрещаю тебе искать то, чего нет.</div>;
    }

    const bouquetReviews = reviews.filter(r => r.bouquet_id === bouquet.bouquet_id);
    ...
    const price = calculateBouquetPrice(bouquet.bouquet_id, bouquetComponents, componentPrices);
    ...
    return (
        <div className="bouquet-details-container">
            <button className="bouquet-details-back" onClick={() => navigate(-1)}>Назад</button>
            ...
            <div className="bouquet-details-top">
                ...
                <div className="bouquet-details-info">
                    ...
                    <div className="bouquet-details-rating">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="var(--color-blue)" viewBox="0 0 16 16">
                            <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
                        </svg>
                        <span>{averageRating > 0 ? averageRating : "Нет оценок"}</span>
                    </div>
                    
                    <p className="bouquet-details-desc">
                        {bouquet.description || "Описание отсутствует. Но ты и так знаешь, что он идеален."}
                    </p>
                    
                    <div className="bouquet-details-price">{price} ₽</div>
                    
                    <button className="btn-primary bouquet-details-buy" onClick={handleAddToCart}>
                        Добавить в корзину
                    </button>
                </div>
            </div>
            ...
            <div className="bouquet-details-reviews">
                ...
                {bouquetReviews.length === 0 ? (
                    <p className="bouquet-details-noreviews">Пока никто не осмелился оставить отзыв.</p>
                ) : (
                    <div className="bouquet-details-reviews-list">
                        {bouquetReviews.map(r => (
                            <div key={r.review_id} className="bouquet-details-review-card">
